output$barChart <- renderDataTable({
...
output$cellMap <-renderDataTable({
  ...
  dp_cells <- class_x %>%
    ...
    select(!matches(marker1), !matches(marker2))
  ...
  # # }
  marker_ob_ids <- as.tibble(comb_markers$`Object Id`)
  ...
  comb_markers <- comb_markers %>%
    bind_rows(x_y_none)
  # final_graph <-(comb_markers %>%
  #                  plot_ly(x=~x, y=~y, mode = 'markers', color = ~marker) %>% 
  #                  add_trace(x_y_none, x=~x, y=~y, mode = 'markers', color = ~marker) %>% 
  #                  layout(paper_bgcolor = '#000000',
  #                         plot_bgcolor = '#000000',
  #                         xaxis = list(showgrid = FALSE, showticklabels = FALSE, showgrid = FALSE),
  #                         yaxis = list(showgrid = FALSE, showticklabels = FALSE, showgrid = FALSE)
                          # ))
